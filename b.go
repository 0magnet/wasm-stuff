...
func main() {
	...
	updateOutput := js.FuncOf(func(this js.Value, p []js.Value) interface{} {
	...
	defer updateOutput.Release()



	radioLorenz := doc.Call("getElementById", "radio-lorenz")
	radioLorenz.Call("addEventListener", "change", updateOutput)
	radioRossler := doc.Call("getElementById", "radio-rossler")
	radioRossler.Call("addEventListener", "change", updateOutput)
	radioChua := doc.Call("getElementById", "radio-chua")
	radioChua.Call("addEventListener", "change", updateOutput)
	radioAizawa := doc.Call("getElementById", "radio-aizawa")
	radioAizawa.Call("addEventListener", "change", updateOutput)
	radioSprott := doc.Call("getElementById", "radio-sprott")
	radioSprott.Call("addEventListener", "change", updateOutput)
	radioLissajou := doc.Call("getElementById", "radio-lissajou")
	radioLissajou.Call("addEventListener", "change", updateOutput)
	radioCube := doc.Call("getElementById", "radio-cube")
	radioCube.Call("addEventListener", "change", updateOutput)
	radioSphere := doc.Call("getElementById", "radio-sphere")
	radioSphere.Call("addEventListener", "change", updateOutput)
...
`
	...
	if !sliderZoom.IsUndefined() {	sliderZoom.Set("textContent", fmt.Sprintf("%.1f", cameraDist)) }
	if !sliderX.IsUndefined() {	sliderX.Set("textContent", fmt.Sprintf("%.1f", rotationX)) }
	if !sliderY.IsUndefined() {	sliderY.Set("textContent", fmt.Sprintf("%.1f", rotationY)) }
	if !sliderZ.IsUndefined() {	sliderZ.Set("textContent", fmt.Sprintf("%.1f", rotationZ)) }
    ...
    rotationY = float32(js.Global().Get("parseFloat").Invoke(rotationControlsY.Get("value")).Float())
    rotationZ = float32(js.Global().Get("parseFloat").Invoke(rotationControlsZ.Get("value")).Float())
		sliderZoom.Set("textContent", fmt.Sprintf("%.1f", cameraDist))
		sliderX.Set("textContent", fmt.Sprintf("%.1f", rotationX))
		sliderY.Set("textContent", fmt.Sprintf("%.1f", rotationY))
		sliderZ.Set("textContent", fmt.Sprintf("%.1f", rotationZ))
		if cameraDist != 0 {
